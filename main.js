...
var DEFAULT_SETTINGS = {
  ...
  logMaxEntries: 50
  // Default to showing 50 log entries
};
var GitLogModal = class extends import_obsidian.Modal {
  // Can be structured logs or a pre-formatted string
  ...
  onOpen() {
    ...
    if (typeof this.logs === "string") {
      ...
      this.logs.forEach((log) => {
        ...
        entryEl.style.borderBottom = "1px dashed var(--background-modifier-border-hover)";
    ...
    new import_obsidian.Setting(contentEl).addButton(
      (btn) => btn.setButtonText("Copy Logs").setIcon("copy").setCta().onClick(async () => {
        let logText = "";
        if (typeof this.logs === "string") {
          logText = this.logs;
        } else if (Array.isArray(this.logs)) {
          logText = this.logs.map(
            (log) => `Commit: ${log.hash}
...
---`
        ...
        if (logText) {
          ...
          new import_obsidian.Notice("Git logs copied to clipboard!");
        } else {
          new import_obsidian.Notice("No logs to copy.");
        }
      })
    ).addButton(
...
var GitSyncPlugin = class extends import_obsidian.Plugin {
  ...
  // Renamed from onSettingsChanged for clarity
  async onload() {
  ...
  async viewGitLog() {
    if (!this.git) {
      new import_obsidian.Notice("Git Sync: Git is not initialized. Cannot view logs.");
      return;
    }
    try {
      ...
      const logData = await this.git.log(logOptions);
      if (logData.all && logData.all.length > 0) {
        new GitLogModal(this.app, logData.all).open();
      } else {
        new import_obsidian.Notice("Git Sync: No log entries found.");
      }
      this.updateStatusBar("Idle");
    } catch (error) {
      console.error("Git Sync: Error fetching Git log", error);
      new import_obsidian.Notice(`Git Sync: Failed to fetch Git log. ${error.message}`);
  ...
  async syncVault() {
    ...
    try {
      ...
      const hasLocalChanges = initialStatus.files.some(
        (file) => file.working_dir !== " " || file.index !== " " && file.index !== "?"
      );
      if (hasLocalChanges) {
        ...
        try {
          ...
          if (stashResult && !stashResult.toLowerCase().includes("no local changes to save")) {
            stashed = true;
      ...
      if (stashed) {
        ...
        try {
          ...
          if (stashPopError.message && stashPopError.message.includes("conflict")) {
            console.error(
      ...
      if (stashed && !(error.message.includes("Conflict applying stashed changes") || error.message.includes("Merge conflict during rebase"))) {
        new import_obsidian.Notice(
  ...
  handleSyncError(error) {
    ...
    if (message) {
      if (message.includes("CONFLICT") || message.includes("conflict") || error.git && error.git.failed && error.git.message && error.git.message.includes("conflict")) {
        errorMessage = "Git Sync: A conflict occurred. Please resolve it manually in your Git client.";
        this.updateStatusBar("Conflict!");
      } else if (message.includes("Host key verification failed") || message.includes("Permission denied")) {
      ...
      } else if (message.includes("Merge conflict during rebase, operation aborted")) {
        errorMessage = "Git Sync: Rebase conflict. Manual resolution needed. Rebase was aborted.";
        this.updateStatusBar("Rebase Conflict!");
      } else if (message.includes("Conflict applying stashed changes")) {
        errorMessage = "Git Sync: Stash conflict. Manual resolution needed. Stash may still be present.";
        this.updateStatusBar("Stash Conflict!");
      } else {
...
var GitSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("View Git Log").setDesc("Display the recent Git commit log for this vault.").addButton(
      (button) => button.setButtonText("View Log").onClick(() => {
        this.plugin.viewGitLog();
      })
    );
    containerEl.createEl("h3", { text: "Advanced" });
    new import_obsidian.Setting(containerEl).setName("Max Log Entries").setDesc("Maximum number of log entries to display in the Git Log view.").addText(
      (text) => text.setPlaceholder(String(DEFAULT_SETTINGS.logMaxEntries)).setValue(String(this.plugin.settings.logMaxEntries)).onChange(async (value) => {
